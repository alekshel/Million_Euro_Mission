...
class TextInterface:
    ...
    def display_market(self) -> str:
        ...
        # Тобто "хвіст крутить собакой" - не гра знає як себе відобразити, а відображення бере все з гри на свій розсуд.
        # Маєте розпорошення коду, а при необхідності змін в грі будете змінювати код і там і тут. 
        # А нове відображення замість простого підключення чи адаптування вимагає значно більше зусиль. 
        # Треба було застосувати Bridge.
...
# Дуже схоже наантіпатерн "Магічна кнопка (Magic pushbutton)" - Кодування логіки реалізації безпосередньо в коді UI, без використання абстракції.
